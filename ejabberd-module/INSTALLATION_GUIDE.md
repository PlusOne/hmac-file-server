...
### 🎯 Overview
This module enables seamless file uploads in XMPP clients by integrating ejabberd with HMAC File Server 3.4.0. Users get zero-configuration file sharing with automatic authentication.
...
- **HMAC File Server 3.4.0** with Bearer token support
- **Network connectivity** between ejabberd and HMAC server

### Step 1: Install HMAC File Server 3.4.0
```bash
# Download and install HMAC File Server
wget https://git.uuxo.net/uuxo/hmac-file-server/releases/v3.4.0/hmac-file-server-linux-amd64
...
[Unit]
Description=HMAC File Server 3.4.0
